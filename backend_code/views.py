            ...
            if json_auth_err:
                ...
                try:
                    ...
                    if not current_customer.is_active or not store_check:
                        return JsonResponse({'Status': False, 'Error': 'Vendor or store is not active'})
                    else:
                        ...
                        if current_customer.seller_vendor_id != data_loaded['vendor_id']:
                            return JsonResponse({'Status': False, 'Error': 'This customer cannot import products for this vendor'})
                        else:
                            skipped = 0
                            ...
                            for item in data_loaded['goods']:
                                check_article_unique = Product.objects.filter(stock_number=item['stock_number']).exclude(delivery_store=current_customer.unique_vendor_id)
                                if check_article_unique:
                                    ...
                                    current_item.save()
                                    prod_params, __ = ProductParameters.objects.update_or_create(pr_id=current_item, defaults={'screen_size': item['parameters']['Диагональ (дюйм)'], 'dimension': item['parameters']['Разрешение (пикс)'], 'RAM': item['parameters']['Встроенная память (Гб)'], 'color': item['parameters']['Цвет']})
                                    prod_params.save()
                            return JsonResponse({'Status': True, 'Message': f'Product list updated. Number of skipped items: {skipped}'})
                except ValueError as err:
